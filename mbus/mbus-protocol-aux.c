...
static const mbus_variable_vif vif_table[] = {
...
ADDAPI int ADDCALL mbus_fixed_normalize(int medium_unit, long medium_value, char **unit_out, double *value_out, char **quantity_out)
{
    double exponent = 0.0;
    int i;
...
mbus_context_serial(const char *device)
{
	...
	#ifndef _WIN32
	if ((serial_data->t = (struct termios *)malloc(sizeof(struct termios))) == NULL)
    {
        snprintf(error_str, sizeof(error_str), "%s: failed to allocate memory for termios\n", __PRETTY_FUNCTION__);
        mbus_error_str_set(error_str);
        free(serial_data);
        free(handle);
        return NULL;
	...
	handle->tcp_timeout_sec = 4ll;
	handle->tcp_timeout_usec = 0ll;
